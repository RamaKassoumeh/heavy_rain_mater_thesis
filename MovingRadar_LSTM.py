...
# Parameters
input_channels = 1  # Number of input channels (for example, RGB channels)
hidden_channels = 64  # Number of hidden channels
kernel_size = (3, 3)  # Kernel size for ConvLSTM
num_layers = 6 # Number of ConvLSTM layers
output_channels = 1  # Number of output channels for the image (e.g., RGB)

model = ConvLSTMModel(input_channels, hidden_channels, kernel_size, num_layers, output_channels)
...
for epoch in range(1, num_epochs + 1):
    ...
    for batch_num, (input, target) in enumerate(train_dataloader, 1):
        ...
        if batch_num%100 ==0:
            ...
            plot_images([input[0,input.shape[1]-1,0],input[0,input.shape[1]-2,0],input[0,input.shape[1]-1,0],input[0,input.shape[1]-2,0],input[0,input.shape[1]-1,0],input[0,input.shape[1]-2,0] ,target[0][0],output[0][0]], 2, 4,epoch,batch_num,'train',folder_name)

...
train_dataset = RadarFilterRainNetSatelliteDataset(
    ...
    transform=model_RainNet.radar_undefined_transform,
    inverse_transform=model_RainNet.radar_undefined_transform,
...
for batch_num, (input_data, target, original_target) in enumerate(train_dataloader, 1):
    counter+=1
    inverse_target=model_RainNet.radar_undefined_inverse_transform(target)
    assert torch.isclose(original_target,inverse_target).tolist(), "tensor1 and tensor2 are not equal"
    input_data=model_RainNet.radar_undefined_inverse_transform(input_data)

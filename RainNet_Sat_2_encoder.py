...
class RainNet(nn.Module):
    ...
    def __init__(self, 
        ...
        self.upsample_1st = nn.Upsample(scale_factor=(2,2,2))
        self.upsample_2nd = nn.Upsample(scale_factor=( 5 / 2,2,2))
        self.upsample_3rd = nn.Upsample(scale_factor=(11/5,2,2))
        ...
        if self.mode == "regression":
            self.last_layer = nn.Sequential(
                nn.Conv3d(2, 1, kernel_size=(11,1,1), padding = 'valid'),
                )
    ...
    def forward(self, x):
        ...
        x=torch.cat((x_r, x_s), dim=1)
        # x = self.conv["5"](self.pool(self.drop(x4s))) # conv5s
        padding=(0,0,0,0,0,x3s.size(2)-x3r.size(2),0,0)
        x3r = F.pad(x3r, padding)
        x = torch.cat((self.upsample_1st(self.drop(x)), x3r,x3s), dim=1) # up6
        padding=(0,0,0,0,0,x2s.size(2)-x2r.size(2),0,0)
        x2r = F.pad(x2r, padding)
        x = torch.cat((self.upsample_2nd(self.conv["5"](x)), x2r,x2s), dim=1) # up7
        padding=(0,0,0,0,0,x1s.size(2)-x1r.size(2),0,0)
        x1r = F.pad(x1r, padding)
        x = torch.cat((self.upsample_3rd(self.conv["6"](x)), x1r,x1s), dim=1) # up8
        # x = torch.cat((self.upsample(self.conv["8"](x)), x1s), dim=1) # up9
        x = self.conv["7"](x) #conv9
        x = self.conv["8"](x) #conv9
